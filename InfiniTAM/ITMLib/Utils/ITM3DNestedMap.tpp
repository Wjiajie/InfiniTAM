...
template<typename T>
bool ITM3DNestedMap<T>::InsertOrdered(int keyLevel2, int keyLevel1, int keyLevel0, T valueLevel0) {
...
template<typename T>
ITM3DNestedMap<T>::ITM3DNestedMap(const char* prefixLevel2, const char* prefixLevel1, const char* prefixLevel0) :
		internalMap(),
...
template<typename T>
std::vector<int> ITM3DNestedMap<T>::GetLevel2Keys() {
	std::vector<int> keys;
...
template<typename T>
bool ITM3DNestedMap<T>::Contains(int keyLevel2, int keyLevel1, int keyLevel0) {
...
template<typename T>
bool ITM3DNestedMap<T>::Contains(int keyLevel2, int keyLevel1, int keyLevel0, T valueLevel0) {
	if (this->internalMap.find(keyLevel2) == (this->internalMap).end()) { return false; }
	auto& valueLevel2 = internalMap[keyLevel2];
...
template<typename T>
bool ITM3DNestedMap<T>::Contains(int keyLevel2, int keyLevel1) {
	if (this->internalMap.find(keyLevel2) == (this->internalMap).end()) { return false; }
	auto& valueLevel2 = internalMap[keyLevel2];
...
template<typename T>
const T* ITM3DNestedMap<T>::GetFirstValue() const {
	if (internalMap.empty()) return nullptr;
	return &(*(*(*this->internalMap.begin()).second.begin()).second.begin()).second;
}

template<typename T>
const T* ITM3DNestedMap<T>::GetLastValue() const {
	if (internalMap.empty()) return nullptr;
	return &(*(--(*(--(*(--this->internalMap.end())).second.end())).second.end())).second;
}

template<typename T>
const T* ITM3DNestedMap<T>::GetValueAfter(int keyLevel2, int keyLevel1, int keyLevel0) const {
...
template<typename T>
const T* ITM3DNestedMap<T>::GetValueBefore(int keyLevel2, int keyLevel1, int keyLevel0) const {
	auto iteratorLevel2 = this->internalMap.find(keyLevel2);
	//check if top-level map contains third-level key, if it doesn't, return null pointer
	if (iteratorLevel2 == (this->internalMap).end()) { return nullptr; }
	//if it does, we have to check the second level to see if we can retrieve the next level1 element
...
template<typename T>
const T* ITM3DNestedMap<T>::GetValueAt(int keyLevel2, int keyLevel1, int keyLevel0) const {
	auto iteratorLevel2 = this->internalMap.find(keyLevel2);
	//check if top-level map contains third-level key, if it doesn't, return null pointer
	if (iteratorLevel2 == (this->internalMap).end()) { return nullptr; }
	//if it does, we have to check the second level to see if we can retrieve the next level1 element
...
template<typename T>
std::vector<T> ITM3DNestedMap<T>::GetValues() const {
	std::vector<T> out;
	for (auto elementLevel2 : internalMap) {
		for (auto elementLevel1 : elementLevel2.second) {
			for (auto elementLevel0 : elementLevel1.second) {
