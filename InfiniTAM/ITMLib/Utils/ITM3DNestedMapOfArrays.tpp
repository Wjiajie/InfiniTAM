...
template<typename T>
bool ITM3DNestedMapOfArrays<T>::InsertOrdered(int keyLevel3, int keyLevel2, int keyLevel1, T valueLevel0) {
...
namespace ITMLib {
template<typename T>
std::ostream& operator<<(std::ostream& stream, const ITM3DNestedMapOfArrays<T>& nestedMap3D) {
...
template<typename T>
ITM3DNestedMapOfArrays<T>::ITM3DNestedMapOfArrays(const ITM3DNestedMapOfArrays<T>& nestedMap3D) {
	for (auto elementLevel3 : nestedMap3D.internalMap) {
		for (auto elementLevel2 : elementLevel3.second) {
			for (auto elementLevel1 : elementLevel2.second) {
...
template<typename T>
ITM3DNestedMapOfArrays<T>::~ITM3DNestedMapOfArrays() {
}
...
ITM3DNestedMapOfArrays<T>& ITM3DNestedMapOfArrays<T>::operator=(ITM3DNestedMapOfArrays& other) {
	ITM3DNestedMapOfArrays tmp(other);
	*this = std::move(tmp);
	return *this;
}
...
template<typename T>
const std::vector<T>* ITM3DNestedMapOfArrays<T>::GetArrayAfter(int keyLevel3, int keyLevel2, int keyLevel1) const {
...
template<typename T>
const std::vector<T>* ITM3DNestedMapOfArrays<T>::GetArrayBefore(int keyLevel3, int keyLevel2, int keyLevel1) const {
	auto iteratorLevel3 = this->internalMap.find(keyLevel3);
	//check if top-level map contains third-level key, if it doesn't, return null pointer
	if (iteratorLevel3 == (this->internalMap).end()) { return nullptr; }
	//if it does, we have to check the second level to see if we can retrieve the next level1 element
...
template<typename T>
const std::vector<T>* ITM3DNestedMapOfArrays<T>::GetArrayAt(int keyLevel3, int keyLevel2, int keyLevel1) const {
	auto iteratorLevel3 = this->internalMap.find(keyLevel3);
	//check if top-level map contains third-level key, if it doesn't, return null pointer
	if (iteratorLevel3 == (this->internalMap).end()) { return nullptr; }
	//if it does, we have to check the second level to see if we can retrieve the next level1 element
...
template<typename T>
std::vector<std::vector<T>> ITM3DNestedMapOfArrays<T>::GetArrays() const {
